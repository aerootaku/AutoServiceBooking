                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card card-accent-theme">
                            <div class="card-body">
                                <h4 class="text-theme">Users Concern and Feedback</h4>
                                ...
                                <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable" width="100%">
                                    ...
                                    <tbody>
                                    <?php
                                    $value = db_select_whereCustom('tbl_customer_concern', $data = array("company_name"=>$_SESSION['company_name']));
                                    if($value->rowCount()>0)
                                    {
                                        while($row=$value->fetch(PDO::FETCH_ASSOC))
                                        {
                                            ...
                                            <tr>
                                                <td><?= $row['lastname'].", ". $row['firstname']. " ".
                                                    $row['middlename'];  ?></td>
                                                <td><?php custom_echo($row['feedback'], 20) ; ?></td>
                                                <td><?php custom_echo($row['suggestions'], 20) ; ?></td>
                                                <td>
                                                    <a href="" data-target="#edit<?= $id; ?>" data-toggle="modal" class="btn
                                                    btn-xs
                                                    btn-info" style="margin-top: -5px"><i class="icon-eye" style="color:
                                                     black;" title="View Feedback"></i> </a>
    ...
    while($row=$value->fetch(PDO::FETCH_ASSOC))
    {
        ...
        <div id="edit<?= $id; ?>" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h6 class="modal-title">View <?= $row['lastname'].", ". $row['firstname']. " ".
                            $row['middlename']; ?> concern & feedback</h6>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">×</span>
                        </button>
                    </div>
                    <form action="?id=<?=$row['id']; ?>" method="POST" id="needs-validation" novalidate="" enctype="multipart/form-data">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="validationCustom05">Feedback</label>
                                <textarea type="text" class="form-control" id="validationCustom05"
                                          placeholder="Feedback"
                                          required="" name="description" rows="5"><?= $row['feedback']; ?></textarea>
                            </div>
                            <div class="form-group">
                                <label for="validationCustom05">Suggestions</label>
                                <textarea type="text" class="form-control" id="validationCustom05"
                                          placeholder="Suggestions"
                                          required="" name="description" rows="5"><?= $row['suggestions']; ?></textarea>
                            </div>
